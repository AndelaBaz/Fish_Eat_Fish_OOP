...
namespace OTTER
{
    ...
    public partial class BGL : Form
    {
        ...
        /* Initialization */
        Glavna mojaRiba;
        Neprijatelj crveni, zuti, piranha;
        Sprite bubble, btnPlay, btnExit;
        ...
        private void SetupGame()
        {
            ...
            setBackgroundPicture("backgrounds\\prvaaa.jpg");
            //none, tile, stretch, center, zoom
            setPictureLayout("stretch");

            //2. add sprites
            
            int yp = r.Next(0, GameOptions.DownEdge);
            
            int yp2 = r.Next(0, GameOptions.DownEdge);
            
            int yp3 = r.Next(0, GameOptions.DownEdge);
            
            mojaRiba = new Glavna("sprites\\moja.png", 0, 0, 5, 0,3);
            mojaRiba.SetSize(10);
            mojaRiba.X = GameOptions.RightEdge / 2 - mojaRiba.Width / 2;
            mojaRiba.Y = GameOptions.DownEdge / 2 - mojaRiba.Heigth / 2;
            
            mojaRiba.RotationStyle = "AllAround";

            



            crveni = new Neprijatelj("sprites\\riba1.png", 0, yp, 5);
            crveni.SetHeading(Sprite.DirectionsType.right);
            crveni.SetSize(10);
            

            zuti = new Neprijatelj("sprites\\zuti.png", 500, yp2, 5);
            zuti.SetHeading(Sprite.DirectionsType.left);
            zuti.SetSize(10);

            piranha = new Neprijatelj("sprites\\piranha.png", 600, yp3, 10);
            
            piranha.SetHeading(Sprite.DirectionsType.left);
            piranha.SetSize(10);

            bubble = new Sprite("sprites\\bubble.png", 450, 420);
            bubble.SetSize(3);
            bubble.SetHeading(Sprite.DirectionsType.up);

            btnPlay = new Sprite("sprites\\playy.png", 150,  10);
            btnPlay.SetSize(100);
            Game.AddSprite(btnPlay);

            btnExit = new Sprite("sprites\\exitttt.png", 270, 250);
            btnExit.SetSize(70);
            Game.AddSprite(btnExit);

            

            Game.AddSprite(bubble);
            Game.AddSprite(mojaRiba);
            Game.AddSprite(crveni);
            Game.AddSprite(zuti);
            Game.AddSprite(piranha);
        ...
        private int RibaPoint()
        {
            while (START)
            {
                mojaRiba.PointToMouse(sensing.Mouse);
                Wait(0.02);
            }
            return 0;
        }

        private int RibaMove()
        {
            while (START)
            {

                mojaRiba.MoveSteps(mojaRiba.Brzina);
                Wait(0.02);
            }
            return 0;
        }

        private int KretanjeNeprijatelja()
        {

            while (START)
            {
                ISPIS = String.Format("Bodovi: {0} \nŽivoti: {1}", mojaRiba.Bodovi.ToString(), mojaRiba.Zivot.ToString());
                bubble.Y -= 8;
                piranha.MoveSteps(piranha.Brzina);
                crveni.MoveSteps(crveni.Brzina);
                zuti.MoveSteps(zuti.Brzina);
                if (crveni.TouchingSprite(mojaRiba))
                {
                    
                    crveni.X = 0;
                    crveni.Y = r.Next(0, GameOptions.DownEdge);
                    mojaRiba.Bodovi++;


                }
                
                if (crveni.TouchingEdge())
                {
                    crveni.X = 0;
                    crveni.Y = r.Next(0, GameOptions.DownEdge);
                }
                if (zuti.TouchingSprite(mojaRiba))
                {
                    zuti.X = 500;
                    zuti.Y = r.Next(0, GameOptions.DownEdge);
                    mojaRiba.Bodovi++;
                ...
                if (zuti.TouchingEdge())
                {
                    zuti.X = 500;
                    zuti.Y = r.Next(0, GameOptions.DownEdge);
                }
                if (piranha.TouchingSprite(mojaRiba))
                {
                    try
                    {
                        mojaRiba.Zivot--;
                        piranha.X = 600;
                        piranha.Y = r.Next(0, GameOptions.DownEdge);
                        mojaRiba.X = 0;
                        mojaRiba.Y = 0;
                    ...
                    {
                        START = false;
                        ISPIS = String.Format("KRAJ");
                    }
                    

                }
                if (piranha.TouchingEdge())
                {
                    piranha.X = 600;
                    piranha.Y = r.Next(0, GameOptions.DownEdge);
                }
                if (bubble.TouchingEdge())
                {
                    bubble.X = 450;
                    bubble.Y = 420;
                }
        ...
        private int Izbornik()
        {
            while (IZBORNIK)
            {
                mojaRiba.SetVisible(false);
                crveni.SetVisible(false);
                zuti.SetVisible(false);
                piranha.SetVisible(false);
                bubble.SetVisible(false);

                if(sensing.MouseDown)
                {
                    if(btnPlay.Clicked(sensing.Mouse))
                    {
                        IZBORNIK = false;
                        START = true;
                        setBackgroundPicture("backgrounds\\pozadina.jpg");
                        setPictureLayout("strech");
                        mojaRiba.SetVisible(true);
                        crveni.SetVisible(true);
                        zuti.SetVisible(true);
                        piranha.SetVisible(true);
                        bubble.SetVisible(true);
                        btnPlay.SetVisible(false);
                        btnExit.SetVisible(false);
                        Game.StartScript(RibaPoint);
                        Game.StartScript(RibaMove);
                        Game.StartScript(KretanjeNeprijatelja);
                        Wait(0.1);
                    }
                    else if(btnExit.Clicked(sensing.Mouse))
                    {
                        Application.Exit();
                    }
                }
